...
#include "qt/epgc/send.h"
#include "qt/epgc/forms/ui_send.h"
#include "qt/epgc/addnewcontactdialog.h"
#include "qt/epgc/qtutils.h"
#include "qt/epgc/sendchangeaddressdialog.h"
#include "qt/epgc/optionbutton.h"
#include "qt/epgc/sendconfirmdialog.h"
#include "qt/epgc/myaddressrow.h"
...
#include "zepgcontroldialog.h"

SendWidget::SendWidget(EPGCGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("EPG");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zEPG");
    setCssProperty(ui->pushRight, "btn-check-right");

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (EPG) or private coins (zEPG)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("EPGC address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 EPG");
    ...
    setCssProperty(coinIcon, "coin-icon-epg");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onEPGSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onEPGSelected(false);});
...
void SendWidget::refreshAmounts() {
    ...
    bool isZepg = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZepg));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (isZepg ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    bool sendEpg = ui->pushLeft->isChecked();

    // request unlock only if was locked or unlocked for mixing:
    // this way we let users unlock by walletpassphrase or by menu
    // and make many transactions while unlocking through this dialog
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendEpg ? AskPassphraseDialog::Context::Send_EPG : AskPassphraseDialog::Context::Send_zEPG, true)){
        // Unlock wallet was cancelled
        inform(tr("Cannot send, wallet locked"));
        return;
    }

    if((sendEpg) ? send(recipients) : sendZepg(recipients)) {
...
bool SendWidget::sendZepg(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;
    ...
    // use mints from zEPG selector if applicable
    std::vector<CMintMeta> vMintsToFetch;
    std::vector<CZerocoinMint> vMintsSelected;
    if (!ZEpgControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZEpgControlDialog::GetSelectedMints();
    ...
    if (walletModel->sendZepg(
        ...
        inform(tr("zEPG transaction sent!"));
        ZEpgControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZEPG_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zEPG require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zEPG transaction
...
void SendWidget::onCoinControlClicked(){
    if(isEPG){
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any EPG to select."));
        }
    }else{
        if (walletModel->getZerocoinBalance() > 0) {
            ZEpgControlDialog *zEpgControl = new ZEpgControlDialog(this);
            zEpgControl->setModel(walletModel);
            zEpgControl->exec();
            ui->btnCoinControl->setActive(!ZEpgControlDialog::setSelectedMints.empty());
            zEpgControl->deleteLater();
        } else {
            inform(tr("You don't have any zEPG in your balance to select."));
        }
    }
}
...
void SendWidget::onEPGSelected(bool _isEPG){
    isEPG = _isEPG;
    setCssProperty(coinIcon, _isEPG ? "coin-icon-epg" : "coin-icon-zepg");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress epgAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(epgAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(epgAdd.Get(), dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
