...
#include "qt/epgc/settings/settingsfaqwidget.h"
...
EPGCGUI::EPGCGUI(const NetworkStyle* networkStyle, QWidget* parent) :
        QMainWindow(parent),
        clientModel(0){
...
void EPGCGUI::createActions(const NetworkStyle* networkStyle){
...
void EPGCGUI::connectActions() {
    ...
    connect(topBar, &TopBar::showHide, this, &EPGCGUI::showHide);
    connect(topBar, &TopBar::themeChanged, this, &EPGCGUI::changeTheme);
    connect(topBar, &TopBar::onShowHideColdStakingChanged, navMenu, &NavMenuWidget::onShowHideColdStakingChanged);
    connect(settingsWidget, &SettingsWidget::showHide, this, &EPGCGUI::showHide);
    connect(sendWidget, &SendWidget::showHide, this, &EPGCGUI::showHide);
    connect(receiveWidget, &ReceiveWidget::showHide, this, &EPGCGUI::showHide);
    connect(addressesWidget, &AddressesWidget::showHide, this, &EPGCGUI::showHide);
    connect(privacyWidget, &PrivacyWidget::showHide, this, &EPGCGUI::showHide);
    connect(masterNodesWidget, &MasterNodesWidget::showHide, this, &EPGCGUI::showHide);
    connect(masterNodesWidget, &MasterNodesWidget::execDialog, this, &EPGCGUI::execDialog);
    connect(coldStakingWidget, &ColdStakingWidget::showHide, this, &EPGCGUI::showHide);
    connect(coldStakingWidget, &ColdStakingWidget::execDialog, this, &EPGCGUI::execDialog);
    connect(settingsWidget, &SettingsWidget::execDialog, this, &EPGCGUI::execDialog);
}


void EPGCGUI::createTrayIcon(const NetworkStyle* networkStyle) {
#ifndef Q_OS_MAC
    trayIcon = new QSystemTrayIcon(this);
    QString toolTip = tr("EPGC Core client") + " " + networkStyle->getTitleAddText();
...
void EPGCGUI::setClientModel(ClientModel* clientModel) {
...
void EPGCGUI::createTrayIconMenu() {
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void EPGCGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void EPGCGUI::changeEvent(QEvent* e)
{
...
void EPGCGUI::closeEvent(QCloseEvent* event)
{
#ifndef Q_OS_MAC // Ignored on Mac
...
void EPGCGUI::messageInfo(const QString& text){
...
void EPGCGUI::message(const QString& title, const QString& message, unsigned int style, bool* ret) {
    QString strTitle =  tr("EPGC Core"); // default title
...
bool EPGCGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn){
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("EPGC Core"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
...
void EPGCGUI::showNormalIfMinimized(bool fToggleHidden) {
    if (!clientModel)
        return;
...
void EPGCGUI::toggleHidden() {
    showNormalIfMinimized(true);
}

void EPGCGUI::detectShutdown() {
    if (ShutdownRequested()) {
        if (rpcConsole)
            rpcConsole->hide();
        qApp->quit();
...
void EPGCGUI::goToSend(){
    showTop(sendWidget);
}

void EPGCGUI::goToAddresses(){
    showTop(addressesWidget);
}

void EPGCGUI::goToPrivacy(){
    showTop(privacyWidget);
}

void EPGCGUI::goToMasterNodes(){
    showTop(masterNodesWidget);
}

void EPGCGUI::goToColdStaking(){
    showTop(coldStakingWidget);
}

void EPGCGUI::goToSettings(){
    showTop(settingsWidget);
}

void EPGCGUI::goToReceive(){
    showTop(receiveWidget);
}

void EPGCGUI::showTop(QWidget* view){
...
void EPGCGUI::changeTheme(bool isLightTheme){
...
void EPGCGUI::resizeEvent(QResizeEvent* event){
    // Parent..
...
void EPGCGUI::showHide(bool show){
...
int EPGCGUI::getNavWidth(){
    return this->navMenu->width();
}

void EPGCGUI::openFAQ(int section){
    showHide(true);
...
#ifdef ENABLE_WALLET
bool EPGCGUI::addWallet(const QString& name, WalletModel* walletModel)
{
    ...
    // Connect actions..
    connect(privacyWidget, &PrivacyWidget::message, this, &EPGCGUI::message);
    connect(masterNodesWidget, &MasterNodesWidget::message, this, &EPGCGUI::message);
    connect(coldStakingWidget, &MasterNodesWidget::message, this, &EPGCGUI::message);
    connect(topBar, &TopBar::message, this, &EPGCGUI::message);
    connect(sendWidget, &SendWidget::message,this, &EPGCGUI::message);
    connect(receiveWidget, &ReceiveWidget::message,this, &EPGCGUI::message);
    connect(addressesWidget, &AddressesWidget::message,this, &EPGCGUI::message);
    connect(settingsWidget, &SettingsWidget::message, this, &EPGCGUI::message);
...
bool EPGCGUI::setCurrentWallet(const QString& name) {
    // Single wallet supported.
    return true;
}

void EPGCGUI::removeAllWallets() {
    // Single wallet supported.
}

void EPGCGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address) {
...
static bool ThreadSafeMessageBox(EPGCGUI* gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void EPGCGUI::subscribeToCoreSignals()
{
    // Connect signals to client
    uiInterface.ThreadSafeMessageBox.connect(boost::bind(ThreadSafeMessageBox, this, _1, _2, _3));
}

void EPGCGUI::unsubscribeFromCoreSignals()
{
