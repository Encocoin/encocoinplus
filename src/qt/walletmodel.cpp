...
CAmount WalletModel::getRequiredMasternodeCollateral(int nTier) const
{
    return Params().GetRequiredMasternodeCollateral(chainActive.Height(), nTier);
}

CAmount WalletModel::getMinMasternodeCollateral() const
{
    return masternodePayments.GetMinMasternodeCollateral(chainActive.Height());
}
...
void WalletModel::emitBalanceChanged()
{
    ...
    Q_EMIT balanceChanged(cachedBalance, cachedUnconfirmedBalance, cachedImmatureBalance, cachedLockedCollateralBalance,
...
void WalletModel::checkBalanceChanged()
{
    ...
    if (cachedBalance != newBalance || cachedUnconfirmedBalance != newUnconfirmedBalance || cachedImmatureBalance != newImmatureBalance || cachedLockedCollateralBalance != newLockedCollateralBalance ||
        ...
        Q_EMIT balanceChanged(newBalance, newUnconfirmedBalance, newImmatureBalance, newLockedCollateralBalance,
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    Q_FOREACH (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered epgc address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > sporkManager.GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            Q_EMIT message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 EPG.").arg(sporkManager.GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        Q_FOREACH (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal epgc:URI (epgc:XyZ...?message=example)
            {
...
bool WalletModel::createZepgSpend(
        CWalletTx &wtxNew,
...
bool WalletModel::convertBackZepg(
        CAmount value,
        std::vector<CZerocoinMint> &vMintsSelected,
        bool fMintChange,
        bool fMinimizeChange,
...
static void NotifyzEPGReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzEPGReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzEPGReset.connect(boost::bind(NotifyzEPGReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzEPGReset.disconnect(boost::bind(NotifyzEPGReset, this));
...
bool WalletModel::updateAddressBookPurpose(const QString &addressStr, const std::string& purpose) {
    ...
    if (address.IsStakingAddress())
        return error("Invalid EPGC address, cold staking address");
    CKeyID keyID;
...
bool WalletModel::getKeyId(const CBitcoinAddress& address, CKeyID& keyID) {
    if (!address.IsValid())
        return error("Invalid EPGC address");

    if (!address.GetKeyID(keyID))
        return error("Unable to get KeyID from EPGC address");
...
{
    setMints.clear();
    setMints = pwalletMain->zepgTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
